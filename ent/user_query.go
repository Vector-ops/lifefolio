...
func (uq *UserQuery) QueryMedicalrecord() *MedicalRecordQuery {
	query := (&MedicalRecordClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMedicalrecord: uq.withMedicalrecord.Clone(),
...
func (uq *UserQuery) WithMedicalrecord(opts ...func(*MedicalRecordQuery)) *UserQuery {
	query := (&MedicalRecordClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withMedicalrecord = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMedicalrecord; query != nil {
		if err := uq.loadMedicalrecord(ctx, query, nodes,
			func(n *User) { n.Edges.Medicalrecord = []*MedicalRecord{} },
			func(n *User, e *MedicalRecord) { n.Edges.Medicalrecord = append(n.Edges.Medicalrecord, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadMedicalrecord(ctx context.Context, query *MedicalRecordQuery, nodes []*User, init func(*User), assign func(*User, *MedicalRecord)) error {
...
func (uq *UserQuery) loadInstitution(ctx context.Context, query *InstitutionQuery, nodes []*User, init func(*User), assign func(*User, *Institution)) error {
	...
	nodeids := make(map[uuid.UUID][]*User)
	for i := range nodes {
		if nodes[i].institution_doctor == nil {
			continue
		}
		fk := *nodes[i].institution_doctor
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "institution_doctor" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
